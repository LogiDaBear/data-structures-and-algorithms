...
let biggerThanLuke = (arr) => {
  let lukeMass = Number(arr.find(char => char.name === 'Luke Skywalker').mass);
  let biggerChars = arr.filter(char => Number(char.mass) > lukeMass).map(char => char.name);
  return biggerChars.join(' - ');
};
