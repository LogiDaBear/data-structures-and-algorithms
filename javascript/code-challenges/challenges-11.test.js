...
let findMaleAndFemale = (data) => {
    const filtered = data.filter(character => character.gender === 'male' || character.gender === 'female');
    const names = filtered.map(character => character.name);
...
let findShortest = (data) => {
    ...
    return data.reduce((shortest, current) => {
      if (!shortest || parseInt(current.height) < parseInt(shortest.height)) {
        return current;
      }
      return shortest;
    }, null).name;
